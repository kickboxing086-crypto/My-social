...
const regex = /\{msg\.type === 'user' && \(\s*<div className="flex flex-col bg-black\/40 border border-emerald-900\/20 p-3 rounded-sm w-fit max-w-\[80%\] relative">[\s\S]*?(?=<\/div>\s*\}\s*<\/div>\s*\}\s*<\/div>\s*\)\)\s*<div ref=\{messagesEndRef\})/;
