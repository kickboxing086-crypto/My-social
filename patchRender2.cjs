...
const regex = /\{msg\.type === 'user' && \(\s*<div className=\{`flex flex-col border p-3 rounded-sm w-fit max-w-\[88%\] sm:max-w-\[80%\] relative group mt-2[\s\S]*?(?=<\/div>\s*\}\s*<\/div>\s*\)\)\s*<div ref=\{messagesEndRef\})/;
...
if(match) {
  ...
  console.log('patched messages display');
} else {
